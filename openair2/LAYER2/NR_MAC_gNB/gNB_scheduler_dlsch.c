                ...
                SLsched_t *SL_sched)
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    // if (bytes == 0 && frame != (sched_ctrl->ta_frame + 100) % 1024)
      continue;
    ...
    if (bo->harq_round_max == 1)
      ...
      sched_pdsch->mcs = 26; //modified_mcs; //get_mcs_from_bler(bo, stats, &sched_ctrl->dl_bler_stats, max_mcs, frame); //NEW
